...
func (s *MySQLStore) GetLearnerProfile(ctx context.Context, userID string) (string, error) {
	return scanStringOrEmpty(ctx, s.rw, "get learner profile",
...
func (s *MySQLStore) SaveLearnerProfile(ctx context.Context, userID, profile string) error {
	return saveLearnerProfile(ctx, workguard.Executor(ctx, s.rw), userID, profile)
}

func saveLearnerProfile(ctx context.Context, db execer, userID, profile string) error {
	...
	if _, err := db.ExecContext(ctx, `
		INSERT INTO `+settingsTable+` (user_id, interlocutor_style, learner_profile, updated_at, learner_profile_revision)
		VALUES (?, '', ?, UNIX_TIMESTAMP(), 1)
		ON DUPLICATE KEY UPDATE learner_profile = VALUES(learner_profile), learner_profile_revision = learner_profile_revision + 1, updated_at = VALUES(updated_at)
